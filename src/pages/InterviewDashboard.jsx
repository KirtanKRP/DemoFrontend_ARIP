...
function CustomSelect({ value, onChange, options }) {
    ...
    return (
        <div style={{ position: 'relative', width: '100%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0', background: 'transparent' }} onClick={(e) => { e.stopPropagation(); setOpen(!open); }}>
            <span style={{flex: 1, padding: 0}}>{value}</span>
            <Icon name="chevronDown" size={18} />
            {open && (
                <div style={{
                    position: 'absolute', top: 'calc(100% + 18px)', left: '-42px', right: '-12px',
                    background: '#fff', border: '1px solid #eaeaea', borderRadius: '12px',
                    boxShadow: '0 8px 30px rgba(0,0,0,0.12)', zIndex: 100, padding: '6px',
                    display: 'flex', flexDirection: 'column', gap: '2px', cursor: 'default'
                }} onClick={e => e.stopPropagation()}>
                    {options.map(opt => (
                        <div key={opt} 
                             style={{
                                 padding: '10px 14px', borderRadius: '8px', cursor: 'pointer',
                                 background: opt === value ? '#f9f9f9' : 'transparent',
                                 color: opt === value ? '#000' : '#444',
                                 fontWeight: opt === value ? '600' : '400',
                                 transition: 'all 0.15s ease', fontSize: '14px',
                                 display: 'flex', alignItems: 'center'
                             }}
                             onMouseEnter={e => e.currentTarget.style.background = '#f9f9f9'}
                             onMouseLeave={e => e.currentTarget.style.background = opt === value ? '#f9f9f9' : 'transparent'}
...
function ManualForm({ onBack, onContinue }) {
    ...
    const [form, setForm] = useState({ role: user?.role || '', techStack: '', years: '3', language: user?.language || 'English' });
    ...
    return <div className="manual-form"><header className="manual-heading"><span className="round-icon"><Icon name="sliders" size={31} /></span><div><h2 className="manual-heading-title">Manual Setup</h2><p>Enter your details to customize your interview experience.</p></div></header><div className="fields-grid">{field('Target Role', 'role', 'briefcase', true, { placeholder: 'e.g. Senior Frontend Engineer' })}{field('Technology Stack', 'techStack', 'tag', false, { placeholder: 'e.g. React, Node.js, Python' })}{field('Years of Experience', 'years', 'chart', true, { placeholder: 'e.g. 2', inputMode: 'numeric', type: 'text' })}<label className="manual-field">Preferred Language<span><Icon name="globe" /><CustomSelect value={form.language} onChange={v => setForm({ ...form, language: v })} options={['English', 'Hindi', 'Gujarati', 'Spanish', 'French']} /></span></label></div>{error && <p className="form-error manual-error">{error}</p>}<div className="manual-actions"><button className="secondary-button" onClick={onBack}><Icon name="arrowLeft" size={18} /> Back</button><button className="black-button continue" onClick={submit}>Continue <Icon name="arrowRight" size={18} /></button></div></div>
}
...
export default function InterviewDashboard() {
    ...
    const goManual = () => { setState('manual'); scrollToTop(); };
    const goUpload = () => { setState('upload'); scrollToTop(); };
    ...
    return <div className="dashboard-shell"><Sidebar active="Start Interview" /><main className="dashboard-main"><div className="dashboard-content">
        ...
        {isManual
            ? <button type="button" className="manual-card" onClick={goUpload}>
                ...
                <ResumeArt />
                <b style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>UPLOAD RESUME <Icon name="arrowRight" size={18} /></b>
            </button>
            : <button type="button" className="manual-card" onClick={goManual}>
                ...
                <b style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>MANUAL SETUP <Icon name="arrowRight" size={18} /></b>
            </button>
        }
